...
manual_assert_file_exists() {
  ...
  if [[ "$use_sudo" == "true" ]]; then
    if sudo test ! -f "$filepath"; then
      ERROR "The file: $filepath does not exist, not even with sudo, even though"
      ERROR " one would expect it does exist."
      exit 31
    fi
  elif test ! -f "$filepath"; then
    ERROR "The file: $filepath does not exist, even though one would expect it"
    ERROR " does."
    exit 31
  fi
}
...
manual_assert_dir_exists() {
  ...
  if [[ "$use_sudo" == "true" ]]; then
    if sudo test ! -d "$dirpath"; then
      ERROR "The dir: $dirpath does not exist, not even with sudo, even though"
      ERROR " one would expect it does exist."
      exit 31
    fi
  elif test ! -d "$dirpath"; then
    ERROR "The dir: $dirpath does not exist, even though one would expect it"
    ERROR " does."
    exit 31
  fi
}
