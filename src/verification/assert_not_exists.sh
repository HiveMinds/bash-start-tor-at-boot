...
manual_assert_file_not_exists() {
  ...
  if [[ "$use_sudo" == "true" ]]; then
    if sudo test -f "$filepath"; then
      ERROR "The file: $filepath still exists, even though"
      ERROR " one would expect it does not exist."
      exit 31
    fi
  elif test -f "$filepath"; then
    ERROR "The file: $filepath does still exists, even though one would expect"
...
manual_assert_dir_not_exists() {
  ...
  if [[ "$use_sudo" == "true" ]]; then
    if sudo test -d "$dirpath"; then
      ERROR "The dir: $dirpath still exists, even though"
      ERROR " one would expect it does not exist."
      exit 31
    fi
  elif test -d "$dirpath"; then
    ERROR "The dir: $dirpath does still exists, even though one would expect"
    ERROR " it does not exist."
    exit 31
  fi
}
